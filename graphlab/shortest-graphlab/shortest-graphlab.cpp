...
{
    int dist;
    min_distance_type(int dist = inf) : dist(dist){}
...
class shortest : public graphlab::ivertex_program<graph_type, graphlab::empty,
...
public:
               ...
               const graphlab::empty& empty)
    {
        ...
        else
        {
            ...
            {
                if (vertex.data().arrivalTime > min_dist)
                    vertex.data().arrivalTime = min_dist;
            }
            else
            {
            	if (min_dist < vertex.data().arrivalTime)
            	{
		            changed = true;
		            vertex.data().arrivalTime = min_dist;
		        }
                 ...
                 edge_type& edge) const
    {
        ...
        int newd;
        if (edge.data().w == -1)
            newd = vertex.data().arrivalTime;
        else
            newd = vertex.data().arrivalTime + edge.data().w;
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < QUERY_SIZE; ++ i)
    {
		...
		graphlab::omni_engine<shortest> engine(dc, graph, exec_type);
		engine.signal_all();
		engine.start();
